...
package body Bindings.Rlite.Msg.Flow is
   ...
   function Serialize (Self : in Request) return Byte_Buffer is
      ...
      Flow_Spec : constant Byte_Buffer (1 .. Self.Flow_Spec'Size / 8) with
        Address => Self.Flow_Spec'Address, Import, Volatile;

      Upper_Ipcp_Id :
        constant Byte_Buffer (1 .. Self.Upper_Ipcp_Id'Size / 8) with
        Address => Self.Upper_Ipcp_Id'Address, Import, Volatile;

      Local_Port : constant Byte_Buffer (1 .. Self.Local_Port'Size / 8) with
        Address => Self.Local_Port'Address, Import, Volatile;

      Local_Cep : constant Byte_Buffer (1 .. Self.Local_Cep'Size / 8) with
        Address => Self.Local_Cep'Address, Import, Volatile;

      Uid : constant Byte_Buffer (1 .. Self.Uid'Size / 8) with
        Address => Self.Uid'Address, Import, Volatile;

      Cookie : constant Byte_Buffer (1 .. Self.Cookie'Size / 8) with
        Address => Self.Cookie'Address, Import, Volatile;

      Local_Appl_Size : constant Unsigned_16 :=
        Unsigned_16 (Used_Size (Self.Local_Appl));

      Local_Appl_Size_Ptr :
        constant Byte_Buffer (1 .. Local_Appl_Size'Size / 8) with
        Address => Local_Appl_Size'Address, Import, Volatile;

      Local_Appl_Ptr : constant Byte_Buffer :=
        To_Packed_Buffer (Self.Local_Appl);

      Remote_Appl_Size : constant Unsigned_16 :=
        Unsigned_16 (Used_Size (Self.Remote_Appl));

      Remote_Appl_Size_Ptr :
        constant Byte_Buffer (1 .. Remote_Appl_Size'Size / 8) with
        Address => Remote_Appl_Size'Address, Import, Volatile;

      Remote_Appl_Ptr : constant Byte_Buffer :=
        To_Packed_Buffer (Self.Remote_Appl);
      ...
      Serialized_Msg : constant Byte_Buffer :=
        Hdr_Ptr & Flow_Spec & Upper_Ipcp_Id & Local_Port & Local_Cep & Uid &
        Cookie & Local_Appl_Size_Ptr & Local_Appl_Ptr & Remote_Appl_Size_Ptr &
        Remote_Appl_Ptr & DIF_Name_Size_Ptr & DIF_Name_Ptr;
   begin
   ...
   function Serialize (Self : Response) return Byte_Buffer is
      ...
      Kevent_Id : constant Byte_Buffer (1 .. Self.Kevent_Id'Size / 8) with
        Address => Self.Kevent_Id'Address, Import, Volatile;
      ...
      Port_Id : constant Byte_Buffer (1 .. Self.Port_Id'Size / 8) with
        Address => Self.Port_Id'Address, Import, Volatile;

      Response : constant Byte_Buffer (1 .. Self.Response'Size / 8) with
        Address => Self.Response'Address, Import, Volatile;

      Pad1 : constant Byte_Buffer (1 .. Self.Pad1'Size / 8) with
        Address => Self.Pad1'Address, Import, Volatile;

      Cep_Id : constant Byte_Buffer (1 .. Self.Cep_Id'Size / 8) with
        Address => Self.Cep_Id'Address, Import, Volatile;

      Serialized_Msg : constant Byte_Buffer :=
        Hdr_Ptr & Kevent_Id & Ipcp_Id & Upper_Ipcp_Id & Port_Id & Response &
        Pad1 & Cep_Id;
   begin
      return Serialized_Msg;
   end Serialize;
   ...
   begin
      ...
      Self.Kevent_Id :=
        Buffer_To_Unsigned_32 (Msg_Data (Offset .. Offset + 3));
      ...
      Self.Flowspec :=
        Buffer_To_Flow_Spec
          (Msg_Data (Offset .. Offset + Flow.RINA_Flow_Spec'Size / 8));
      Offset := Offset + Flow.RINA_Flow_Spec'Size / 8;

      --  Local_Appl decoding
      declare
         --  Covert these bytes to a 16-bit u16
         Local_Appl_Length : constant Unsigned_16 :=
           Buffer_To_Unsigned_16 (Msg_Data (Offset .. Offset + 1));
--  Now we know the length of the Appl_Name string, pull it out of the buffer
         Local_Appl : constant String :=
           Buffer_To_String
             (Msg_Data
                (Offset + 2 .. Offset + 2 + Integer (Local_Appl_Length)));
      begin
         Offset := Offset + Unsigned_16'Size / 8;
      ...
      declare
         --  Covert these bytes to a 16-bit u16
         Remote_Appl_Length : constant Unsigned_16 :=
           Buffer_To_Unsigned_16 (Msg_Data (Offset .. Offset + 1));
--  Now we know the length of the Appl_Name string, pull it out of the buffer
         Remote_Appl : constant String :=
           Buffer_To_String
             (Msg_Data
                (Offset + 2 .. Offset + 2 + Integer (Remote_Appl_Length)));
      begin
         Offset := Offset + Unsigned_16'Size / 8;
      ...
      declare
         --  Covert these bytes to a 16-bit u16
         DIF_Name_Length : constant Unsigned_16 :=
           Buffer_To_Unsigned_16 (Msg_Data (Offset .. Offset + 1));
--  Now we know the length of the Appl_Name string, pull it out of the buffer
         DIF_Name : constant String :=
           Buffer_To_String
             (Msg_Data (Offset + 2 .. Offset + 2 + Integer (DIF_Name_Length)));
      begin
         Offset := Offset + Unsigned_16'Size / 8;
