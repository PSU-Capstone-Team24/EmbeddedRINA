...
package body Bindings.Rlite.Msg.IPCP is
   ...
   overriding function Serialize (Self : in Config) return Byte_Buffer is
      ...
      Name_Size : constant Unsigned_16 := Unsigned_16 (Used_Size (Self.Name));

      Name_Size_Ptr : constant Byte_Buffer (1 .. Name_Size'Size / 8) with
        Address => Name_Size'Address, Import, Volatile;

      Name_Ptr : constant Byte_Buffer := To_Packed_Buffer (Self.Name);

      Value_Size : constant Unsigned_16 :=
        Unsigned_16 (Used_Size (Self.Value));

      Value_Size_Ptr : constant Byte_Buffer (1 .. Value_Size'Size / 8) with
        Address => Value_Size'Address, Import, Volatile;

      Value_Ptr : constant Byte_Buffer := To_Packed_Buffer (Self.Value);

      Serialized_Msg : constant Byte_Buffer :=
        Hdr_Ptr & IPCP_Id & Pad1 & Name_Size_Ptr & Name_Ptr & Value_Size_Ptr &
        Value_Ptr;
   begin
      return Serialized_Msg;
   end Serialize;

   overriding function Serialize (Self : in Enroll) return Byte_Buffer is
      Hdr_Ptr : constant Byte_Buffer (1 .. Self.Hdr'Size / 8) with
        Address => Self.Hdr'Address, Import, Volatile;
      ...
      Neigh_Name_Size : constant Unsigned_16 :=
        Unsigned_16 (Used_Size (Self.Neigh_Name));

      Neigh_Name_Size_Ptr :
        constant Byte_Buffer (1 .. Neigh_Name_Size'Size / 8) with
        Address => Neigh_Name_Size'Address, Import, Volatile;

      Neigh_Name_Ptr : constant Byte_Buffer :=
        To_Packed_Buffer (Self.Neigh_Name);
      ...
      Supp_DIF_Name_Size : constant Unsigned_16 :=
        Unsigned_16 (Used_Size (Self.Supp_DIF_Name));

      Supp_DIF_Name_Size_Ptr :
        constant Byte_Buffer (1 .. Supp_DIF_Name_Size'Size / 8) with
        Address => Supp_DIF_Name_Size'Address, Import, Volatile;

      Supp_DIF_Name_Ptr : constant Byte_Buffer :=
        To_Packed_Buffer (Self.Supp_DIF_Name);

      Serialized_Msg : constant Byte_Buffer :=
        Hdr_Ptr & IPCP_Name_Size_Ptr & IPCP_Name_Ptr & Neigh_Name_Size_Ptr &
        Neigh_Name_Ptr & DIF_Name_Size_Ptr & DIF_Name_Ptr &
        Supp_DIF_Name_Size_Ptr & Supp_DIF_Name_Ptr;
   begin
      return Serialized_Msg;
   end Serialize;
