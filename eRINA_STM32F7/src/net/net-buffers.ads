...
package Net.Buffers is
   ...
   procedure Transfer (To : in out Buffer_Type; From : in out Buffer_Type) with
     Pre => not From.Is_Null, Post => From.Is_Null and not To.Is_Null;
   ...
   procedure Switch (To : in out Buffer_Type; From : in out Buffer_Type) with
   ...
   procedure Get_String (Buf : in out Buffer_Type; Into : out String) with
     Pre => not Buf.Is_Null;

   --  Skip a number of bytes in the buffer, moving the buffer position <tt>Size<tt> bytes ahead.
   procedure Skip (Buf : in out Buffer_Type; Size : in Net.Uint16) with
     Pre => not Buf.Is_Null;
   ...
   function Ethernet
     (Buf : in Buffer_Type) return Net.Headers.Ether_Header_Access with
     Pre => not Buf.Is_Null;

   --  Get access to the ARP packet.
   function Arp
     (Buf : in Buffer_Type) return Net.Headers.Arp_Packet_Access with
     Pre => not Buf.Is_Null;

   --  Get access to the EFCP packet.
   function EFCP
     (Buf : in Buffer_Type) return Net.Headers.EFCP_Packet_Access with
     Pre => not Buf.Is_Null;
   ...
   procedure Insert (Into : in out Buffer_List; Buf : in out Buffer_Type) with
     Pre => not Buf.Is_Null, Post => Buf.Is_Null and not Is_Empty (Into);
   ...
   procedure Peek (From : in out Buffer_List; Buf : in out Buffer_Type);
   ...
   procedure Transfer (To : in out Buffer_List; From : in out Buffer_List) with
     Post => Is_Empty (From);

   use type System.Address;

   --  Add a memory region to the buffer pool.
   procedure Add_Region (Addr : in System.Address; Size : in Uint32) with
     Pre =>
      Size mod NET_ALLOC_SIZE = 0 and Size > 0 and Addr /= System.Null_Address;
