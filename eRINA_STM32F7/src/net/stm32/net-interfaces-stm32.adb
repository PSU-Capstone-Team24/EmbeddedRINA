...
package body Net.Interfaces.STM32 is
   ...
   function W is new Ada.Unchecked_Conversion (System.Address, HAL.UInt32);
   ...
   function As_Rx_Ring_Array_Type_Access is new Ada.Unchecked_Conversion
   ...
   protected Receive_Queue with
     Interrupt_Priority => Net.Network_Priority
   is
      ...
      procedure Interrupt with
        Attach_Handler => Ada.Interrupts.Names.ETH_Interrupt, Unreferenced;
   ...
   overriding procedure Send
     (Ifnet : in out STM32_Ifnet; Buf : in out Net.Buffers.Buffer_Type)
   is
      use type Net.Uint64;
   begin
      begin
   ...
   overriding procedure Initialize (Ifnet : in out STM32_Ifnet) is
      ...
      Size : constant Uint32 :=
        Rx_Ring_Array_Type'Length * Buffers.NET_ALLOC_SIZE;
      Addr : System.Address;
   ...
   protected body Transmit_Queue is
      ...
      begin
         ...
         Tx.Desc.Tdes0      :=
           (Own    => 1, Cic => 3, Reserved_2 => 0, Ls => 1, Fs => 1, Ic => 1,
            Cc     => 0, Tch => 1,
      ...
      begin
         ...
         for I in Tx_Ring'Range loop
            Tx_Ring (I).Desc.Tdes0 :=
              (Own    => 0, Ic => 1, Ls => 1, Fs => 1, Dc => 0, Dp => 0,
               Ttse => 0, Tch => 1, Ter => (if I = Tx_Ring'Last then 1 else 0),
               others => <>);
            Tx_Ring (I).Desc.Tdes1 :=
              (Tbs2 => 0, Tbs1 => 0, Reserved_13_15 => 0, Reserved_29_31 => 0);
         ...
         Cur_Tx                       := 0;
         Dma_Tx                       := 0;
         Ethernet_DMA_Periph.DMATDLAR :=
           W (Tx_Ring (Tx_Ring'First).Desc'Address);
   ...
   protected body Receive_Queue is
      ...
      begin
         ...
         Cortex_M.Cache.Clean_DCache
           (Rx.Desc'Address, Integer (Rx.Desc'Size / 8));
         Rx_Count                     := Rx_Count - 1;
      ...
      begin
         ...
         for I in Rx_Ring'Range loop
            ...
            Rx_Ring (I).Desc.Rdes4 :=
              (Reserved_31_14 => 0, Pmt => 0, Ippt => 0, others => 0);
            Rx_Ring (I).Desc.Rdes5 := 0;
            Rx_Ring (I).Desc.Rdes6 := 0;
            Rx_Ring (I).Desc.Rdes7 := 0;
            if I /= Rx_Ring'Last then
               Rx_Ring (I).Desc.Rdes1 :=
                 (Dic => 0, Rbs2 => 0, Rer => 0, Rch => 1,
                  Rbs => UInt13 (Net.Buffers.NET_BUF_SIZE), others => <>);
               Rx_Ring (I).Desc.Rdes3 := W (Rx_Ring (I + 1).Desc'Address);
            else
               Rx_Ring (I).Desc.Rdes1 :=
                 (Dic => 0, Rbs2 => 0, Rer => 1, Rch => 1,
                  Rbs => UInt13 (Net.Buffers.NET_BUF_SIZE), others => <>);
               Rx_Ring (I).Desc.Rdes3 := W (System.Null_Address);
            end if;
         end loop;
         Ethernet_DMA_Periph.DMARDLAR :=
           W (Rx_Ring (Rx_Ring'First).Desc'Address);
         ...
         Ethernet_MAC_Periph.MACCR :=
           (CSTF => True, WD => False, JD => False, IFG => 2#100#,
            CSD  => False, FES => True, ROD => True, LM => False, DM => True,
            IPCO => False, RD => False, APCS => True, BL => 2#10#, DC => True,
            TE   => False, RE => False, others => <>);
         Ethernet_MAC_Periph.MACFFR  := (RA => True, others => <>);
         Ethernet_MAC_Periph.MACHTHR := 0;
         Ethernet_MAC_Periph.MACHTLR := 0;
         Ethernet_MAC_Periph.MACFCR  :=
           (PT   => 0, ZQPD => False, PLT => 0, UPFD => False, RFCE => True,
            TFCE => True, FCB => False, others => <>);
         Ethernet_MAC_Periph.MACVLANTR :=
           (VLANTC => False, VLANTI => 0, others => <>);
         Ethernet_MAC_Periph.MACPMTCSR :=
           (WFFRPR => False, GU => False, WFR => False, MPR => False,
            WFE    => False, MPE => False, PD => False, others => <>);
         ...
         Ethernet_DMA_Periph.DMABMR :=
           (SR   => False, DA => False, DSL => 0, EDFE => False, PBL => 4,
            RTPR => 0, FB => True, RDP => 4, USP => True, FPM => False,
            AAB  => False, MB => False, others => <>);
