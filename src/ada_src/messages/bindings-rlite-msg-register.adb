...
package body Bindings.Rlite.Msg.Register is
   ...
   function Serialize (Self : in Request) return Byte_Buffer is
      ...
      Reg_Ptr        : constant Byte_Buffer(1 .. Self.Reg'Size / 8)
      ...
      Appl_Name_Size : constant Unsigned_16 := Unsigned_16 (Used_Size (Self.Appl_Name));
      
      Appl_Name_Size_Ptr : constant Byte_Buffer(1 .. Appl_Name_Size'Size / 8)
                           with Address => Appl_Name_Size'Address, Import, Volatile;

      Appl_Name_Ptr  : constant Byte_Buffer := To_Packed_Buffer (Self.Appl_Name);
      ...
      Serialized_Msg : constant Byte_Buffer := Hdr_Ptr & Reg_Ptr & Pad1 & Appl_Name_Size_Ptr & Appl_Name_Ptr & Dif_Name_Size_Ptr & Dif_Name_Ptr;
   begin
   ...
   function Serialize (Self : in Move) return Byte_Buffer is
      ...
      Fd_Ptr         : constant Byte_Buffer(1 .. Self.Fd'Size / 8)
                           with Address => Self.Fd'Address, Import, Volatile;

      --  Final serialized concatenated byte buffer in Rlite format
      Serialized_Msg : constant Byte_Buffer := Hdr_Ptr & Ipcp_Id_Ptr & Pad1 & Fd_Ptr;
   begin
      return Serialized_Msg;
   end Serialize;
