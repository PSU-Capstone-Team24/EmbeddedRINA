...
package body Bindings.Rlite.Msg.Flow is
   ...
   function Serialize (Self : in Request) return Byte_Buffer is
      ...
      Flow_Spec      : Byte_Buffer(1 .. Self.Flow_Spec'Size / 8)
                        with Address => Self.Flow_Spec'Address, Import, Volatile;

      Upper_Ipcp_Id  : Byte_Buffer(1 .. Self.Upper_Ipcp_Id'Size / 8)
                        with Address => Self.Upper_Ipcp_Id'Address, Import, Volatile;

      Local_Port     : Byte_Buffer(1 .. Self.Local_Port'Size / 8)
                        with Address => Self.Local_Port'Address, Import, Volatile;
         
      Local_Cep      : Byte_Buffer(1 .. Self.Local_Cep'Size / 8)
                        with Address => Self.Local_Cep'Address, Import, Volatile;

      Uid            : Byte_Buffer(1 .. Self.Uid'Size / 8)
                        with Address => Self.Uid'Address, Import, Volatile;
                     
      Cookie         : Byte_Buffer(1 .. Self.Cookie'Size / 8)
                        with Address => Self.Cookie'Address, Import, Volatile;

      Local_Appl_Size : Unsigned_16 := Unsigned_16 (Used_Size (Self.Local_Appl));

      Local_Appl_Size_Ptr : Byte_Buffer(1 .. Local_Appl_Size'Size / 8)
                           with Address => Local_Appl_Size'Address, Import, Volatile;

      Local_Appl_Ptr   : Byte_Buffer := To_Packed_Buffer (Self.Local_Appl);

      Remote_Appl_Size : Unsigned_16 := Unsigned_16 (Used_Size (Self.Remote_Appl));

      Remote_Appl_Size_Ptr : Byte_Buffer(1 .. Remote_Appl_Size'Size / 8)
                           with Address => Remote_Appl_Size'Address, Import, Volatile;

      Remote_Appl_Ptr   : Byte_Buffer := To_Packed_Buffer (Self.Remote_Appl);
      ...
      Serialized_Msg : Byte_Buffer := Hdr_Ptr & Flow_Spec & Upper_Ipcp_Id & Local_Port & Local_Cep & Uid & Cookie & Local_Appl_Size_Ptr & Local_Appl_Ptr & Remote_Appl_Size_Ptr & Remote_Appl_Ptr & DIF_Name_Size_Ptr & DIF_Name_Ptr;
   begin
