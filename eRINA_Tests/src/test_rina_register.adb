...
package body Test_RINA_Register is
   ...
   exception
      when EXCEPTIONS.DIF_REGISTRATION_FAILURE =>
         Caused_Error := True;
      Assert(Caused_Error, "DIF_Name does not exist in IPCP");
...
-- Test Case 008
   procedure Test_Register_App_Already_Reg (Object : in out Test) is
      ...
      Caused_Error : Boolean := False;
   begin
      Register_Second := RINA_Register (RINA_Dev_FD, App_Name, To_Bounded_String ("TestDIF"), 0);
   exception
      when EXCEPTIONS.DIF_REGISTRATION_FAILURE =>
         Caused_Error := True;
      Assert (Caused_Error, "Application name could be registered twice");
   end Test_Register_App_Already_Reg;

   -- Test Case 009
   procedure Test_Register_AppName_to_DIF (Object : in out Test) is
   App_Name : Bounded_String := To_Bounded_String ("UniqueTestApp");
   RINA_Dev_FD : File_Descriptor := RINA_Open;
   Register_Success : File_Descriptor;
   Caused_Error : Boolean := False;
   begin
      Register_Success := RINA_Register (RINA_Dev_FD, App_Name, To_Bounded_String ("TestDIF"), 0);
   exception
      when EXCEPTIONS.DIF_REGISTRATION_FAILURE =>
         Caused_Error := True;
      Assert (Caused_Error, "Failed to register a unique application name");
