...
package body Test_RINA_Emit_Verdict is
   ...
   begin
      ...
      Test_Suite.Add_Test (Caller.Create
         (Name_023 & " Test Rina Flow Handle Invalid FD", Test_Rina_Flow_Handle_Invalid_FD'Access));
   ...
   -- TC 023
   procedure Test_Rina_Flow_Handle_Invalid_FD (Object : in out Test) is
      Fd : File_Descriptor := Invalid_FD;
   begin
      Fd := OS.Open_Read_Write ("/dev/this_should_fail", OS.Binary);
      Assert(Fd = Invalid_FD, "Device opened that should not be reachable");
   end Test_Rina_Flow_Handle_Invalid_FD;
